...
p_{k} = -B_{k}^{-1} \nabla f_{k}, \quad (3.34)
$$
where the symmetric and positive definite matrix $$B_{k}$$ is updated every iteration by a quasi-Newton updating formula.

__Theorem 3.6__
Suppose that $$f: \mathbb{R}^{n} \rightarrow \mathbb{R}$$ is twice continuously differentiable. Consider the iteration $$x_{k+1} = x_{k} + \alpha_{k} * p_{k}$$, where $$p_{k}$$ is a descent direction and $$\alpha_{k}$$ satisfies the Wolfe conditions (3.6) with $$c_{1} \leq 1/2$$. If the sequence $$\left\{x_{k}\right\}$$ converges to a point $$x^{*}$$ such that $$\nabla f\left(x^{*}\right) = 0$$ and $$\nabla^{2} f\left(x^{*}\right)$$ is positive definite, and if the search direction satisfies
$$
\lim _{k \rightarrow \infty} \frac{\left\|\nabla f_{k}+\nabla^{2} f_{k} p_{k}\right\|}{\left\|p_{k}\right\|}=0, \quad (3.35)
$$
then
- (i) the step length $$\alpha_{k} = 1$$ is admissible for all $$k$$ greater than a certain index $$k_{0}$$; and
- (ii) if $$\alpha_{k} = 1$$ for all $$k > k_{0}$$, $$\left\{x_{k}\right\}$$ converges to $$x^{*}$$ superlinearly.

If $$p_{k}$$ is a quasi-Newton search direction of the form (3.34), then (3.35) is equivalent to
$$
\lim _{k \rightarrow \infty} \frac{\left\|\left(B_{k}-\nabla^{2} f\left(x^{*}\right)\right) p_{k}\right\|}{\left\|p_{k}\right\|}=0. \quad (3.36)
$$

__Theorem 3.7__
Suppose that $$f: \mathbb{R}^{n} \rightarrow \mathbb{R}$$ is twice continuously differentiable. Consider the iteration$$x_{k+1} = x_{k} + \alpha_{k} * p_{k}$$ (that is, the step length $$\alpha_{k}$$ is uniformly $$1$$) and that $$p_{k}$$ is given by (3.34). Let us assume also that $$\left\{x_{k}\right\}$$ converges to a point $$x^{*}$$ such that $$\nabla f\left(x^{*}\right) = 0$$ and $$\nabla^{2} f\left(x^{*}\right)$$ is positive definite. Then $$\left\{x_{k}\right\}$$ converges superlinearly if and only if (3.36) holds.
