...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
    ...
    for (auto &kp : keypoints)
    {
        ...
        double m_sqrt = sqrt(m01 * m01 + m10 * m10) + 1e-18;
        double sin_theta = m01 / m_sqrt;
        double cos_theta = m10 / m_sqrt;
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches)
{
    ...
    for (int i1 = 0; i1 < desc1.size(); ++i1)
    {
        ...
        for (int i2 = 0; i2 < desc2.size(); ++i2)
        {
            ...
            int distance = 0;
            for (int k = 0; k < 8; ++k)
                distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]); // int _mm_popcnt_u32 (unsigned int a): Count the number of bits set to 1 in unsigned 32-bit integer a, and return that count.
